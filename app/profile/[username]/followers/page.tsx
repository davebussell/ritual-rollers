...
export default async function FollowersPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const followers = (rows ?? []).map(r => (r.profiles as unknown) as ProfileRow | null).filter(Boolean) as ProfileRow[]
  ...
  return (
    <div className="min-h-screen bg-zinc-950 px-4 py-12">
      <div className="mx-auto max-w-lg">
        ...
        <h1 className="mb-6 text-xl font-black text-white">{followers.length} Follower{followers.length !== 1 ? 's' : ''}</h1>
        <div className="space-y-2">
          ...
          {followers.length === 0 && <p className="text-zinc-600">No followers yet.</p>}
        </div>
      </div>
    </div>
  )
